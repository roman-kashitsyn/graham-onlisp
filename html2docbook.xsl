...
<xsl:stylesheet version="1.0"
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:call-template name="generate-colspecs">
          ...
          <xsl:with-param name="number" select="concat($number,'    1')"/>
        </xsl:call-template>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template match="html:th|html:td">
    <xsl:variable name="position" select="concat(count(preceding-sibling::*),'   1')"/>
    <entry>
    ...
    <xsl:variable name="new_count">
      <xsl:choose>
        <xsl:when test="$cell/@colspan &gt; 1">
          <xsl:value-of select="concat(number($cell/@colspan),'   ',number($count))"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="concat(number('1'),'  ',number($count))"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <xsl:choose>
